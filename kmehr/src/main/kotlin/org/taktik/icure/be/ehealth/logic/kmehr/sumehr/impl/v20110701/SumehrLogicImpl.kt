...
/**
 * This class implements the SumEHR logic according to the v1.1 standard (20100601) or greater
 */
...
class SumehrLogicImpl(
    ...
    @Qualifier("sumehrExportV1") val sumehrExport: SumehrExport,
    @Qualifier("sumehrImportV1") val sumehrImport: SumehrImport
) : SumehrLogic {
    ...
    override suspend fun isSumehrValid(hcPartyId: String, patient: Patient, patientSecretForeignKeys: List<String>, excludedIds: List<String>, includeIrrelevantInformation: Boolean): SumehrStatus {
        ...
        val servicesByIds = contactLogic.getServices(sumehrServiceIds)
            .toList()
    ...
    override suspend fun getHealthElements(hcPartyId: String, sfks: List<String>, excludedIds: List<String>, includeIrrelevantInformation: Boolean): List<HealthElement> =
        healthcarePartyLogic.getHealthcareParty(hcPartyId)?.let { healthcarePartyLogic.getHcpHierarchyIds(it) }?.let { sumehrExport.getHealthElements(it, sfks, excludedIds, includeIrrelevantInformation).second } ?: listOf()
