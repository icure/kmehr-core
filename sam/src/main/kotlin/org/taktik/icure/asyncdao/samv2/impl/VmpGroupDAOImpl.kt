...
class VmpGroupDAOImpl(couchDbProperties: CouchDbProperties, @Qualifier("drugCouchDbDispatcher") couchDbDispatcher: CouchDbDispatcher, idGenerator: IDGenerator) : InternalDAOImpl<VmpGroup>(VmpGroup::class.java, couchDbProperties, couchDbDispatcher, idGenerator), VmpGroupDAO {
	...
	override fun findVmpGroupsByVmpGroupCode(vmpgCode: String, paginationOffset: PaginationOffset<String>) = flow {
		val client = couchDbDispatcher.getClient(GlobalDatastoreInformation(URI(couchDbProperties.url)))
		emitAll(client.queryView(pagedViewQuery<VmpGroup, String>("by_groupcode", vmpgCode, vmpgCode, paginationOffset, false), String::class.java, Int::class.java, VmpGroup::class.java))
	}

	override fun listVmpGroupsByVmpGroupCodes(vmpgCodes: List<String>) = flow {
		val client = couchDbDispatcher.getClient(GlobalDatastoreInformation(URI(couchDbProperties.url)))
		emitAll(
			client.queryViewIncludeDocs<String, Int, VmpGroup>(
				createQuery<VmpGroup>("by_groupcode")
					.keys(vmpgCodes)
					.reduce(false)
					.includeDocs(true)
			).map { it.doc }
	...
	override fun findVmpGroups(paginationOffset: PaginationOffset<String>) = flow {
		val client = couchDbDispatcher.getClient(GlobalDatastoreInformation(URI(couchDbProperties.url)))
		emitAll(client.queryView(pagedViewQuery<VmpGroup, String>("all", null, "\ufff0", paginationOffset, false), String::class.java, Nothing::class.java, VmpGroup::class.java))
	}

	@View(name = "by_language_label", map = "classpath:js/vmpgroup/By_language_label.js")
	override fun findVmpGroupsByLabel(language: String?, label: String?, paginationOffset: PaginationOffset<List<String>>) = flow {
		val client = couchDbDispatcher.getClient(GlobalDatastoreInformation(URI(couchDbProperties.url)))
		emitAll(
			makeFromTo(label, language).let { (from, to) ->
				client.queryView(
					pagedViewQuery<VmpGroup, ComplexKey>(
						"by_language_label",
						from,
						to,
	...
	override fun listVmpGroupIdsByLabel(language: String?, label: String?) = flow {
		val client = couchDbDispatcher.getClient(GlobalDatastoreInformation(URI(couchDbProperties.url)))
		emitAll(
			makeFromTo(label, language).let { (from, to) ->
				client.queryView<ComplexKey, String>(
					createQuery<VmpGroup>("by_language_label")
						.startKey(from)
						.endKey(to)
						.reduce(false)
						.includeDocs(false)
