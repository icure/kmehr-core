...
import org.taktik.icure.be.samv2v5.entities.*
...
class ActualMedicineImporter {
	...
	suspend fun importActualMedicines(export: ExportActualMedicinesType, vmps: Map<String, VmpStub>, reimbursements: Map<Triple<String?, String?, String?>, MutableSet<Reimbursement>>, couchDbProperties: CouchDbProperties, couchDbDispatcher: CouchDbDispatcher, force: Boolean): Pair<Map<String, String>, Set<String>> {
			...
			amps.forEach { amp ->
				if (!currentAmps.contains(amp.id)) {
					...
					amp.copy(rev = prev.rev).let {
						if (prev != it) {
							log.info("Modified AMP AMP:${it.code}:${it.from} with id ${it.id}")
		...
		val currentSubstances = HashSet(suspendRetry(50) { substanceDAO.getEntityIds().toList() })
		val currentPharmaceuticalForms = HashSet(suspendRetry(50) { pharmaceuticalFormDAO.getEntityIds().toList() })
			...
			try {
				substanceDAO.save(
					it.map {
						Substance(
							...
							name = it.name,
							note = it.note,
							standardSubstances = it.standardSubstances
						)
					}
				)
			} catch (e: Exception) {
				it.forEach {
					try {
						...
						substanceDAO.get(it.id!!)?.let { prev ->
							substanceDAO.update(
								Substance(
									id = it.id,
									rev = prev.rev,
		...
		(pharmaceuticalForms - currentPharmaceuticalForms).values.chunked(100).forEach {
			try {
				pharmaceuticalFormDAO.save(
					it.map {
						PharmaceuticalForm(
							id = it.id!!,
							code = it.code,
							name = it.name,
							standardForms = it.standardForms
						)
					}
				)
			} catch (e: Exception) {
				it.forEach {
					try {
						...
						pharmaceuticalFormDAO.get(it.id!!)?.let { prev ->
							pharmaceuticalFormDAO.update(
								PharmaceuticalForm(
									id = it.id,
									rev = prev.rev,
									code = it.code,
		...
		substances.filterKeys { currentSubstances.contains(it) }.values.chunked(100).forEach {
			val revs = substanceDAO.getEntities(it.map { it.id!! }).toList().fold(mapOf<String, String>()) { acc, it -> acc + (it.id to it.rev!!) }
			it.forEach {
				substanceDAO.update(
					Substance(
						id = it.id!!,
		...
		pharmaceuticalForms.filterKeys { currentPharmaceuticalForms.contains(it) }.values.chunked(100).forEach {
			val revs = pharmaceuticalFormDAO.getEntities(it.map { it.id!! }).toList().fold(mapOf<String, String>()) { acc, it -> acc + (it.id to it.rev!!) }
			it.forEach {
				pharmaceuticalFormDAO.update(
					PharmaceuticalForm(
						id = it.id!!,
		...
		(currentSubstances - substances.keys).chunked(100).forEach { substanceDAO.remove(substanceDAO.getEntities(it)) }
		(currentPharmaceuticalForms - pharmaceuticalForms.keys).chunked(100).forEach { pharmaceuticalFormDAO.remove(pharmaceuticalFormDAO.getEntities(it)) }
		(currentAmps - newAmpIds).chunked(100).forEach { ampDAO.remove(ampDAO.getEntities(it)) }
