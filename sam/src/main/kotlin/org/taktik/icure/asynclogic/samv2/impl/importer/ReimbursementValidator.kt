...
import org.taktik.icure.utils.differences
...
class ReimbursementValidator {
	...
	suspend fun validateReimbursements(export: ExportReimbursementsType, reimbursements: MutableMap<Triple<String?, String?, String?>, MutableSet<Reimbursement>>, couchDbProperties: CouchDbProperties, couchDbDispatcher: CouchDbDispatcher): List<Diff> {
		...
		HashSet<String>(suspendRetry(50) { ampDAO.getEntityIds().toList() }).chunked(100).forEach { ids ->
			diffs.addAll(
				ampDAO.getEntities(ids).fold<Amp, List<Diff>>(listOf()) { acc, amp ->
					...
					if (ampps != amp.ampps) {
						acc + listOf(Diff("Amp[${amp.code}].ampps", ampps.differences(amp.ampps)))
					} else acc
				}.asFlow().toList()
			)
		}
